...
stmmac_eth_start(struct rte_eth_dev *dev)
{
	...
	//stmmac_dump_gregs(dev->data->dev_private);
...
static void
stmmac_mac_link_up(struct stmmac_private *priv, unsigned int mode __rte_unused,
		  phy_interface_t interface __rte_unused, int speed, int duplex,
			...
			int wait_to_complete __rte_unused)
{
	...
	STMMAC_PMD_DEBUG("Port (%d) link is %s\n", dev->data->port_id, "Up");
...
static int
stmmac_store_mac_address(struct rte_eth_dev *dev,
		    struct rte_ether_addr *addr)
{
	rte_ether_addr_copy(addr, &dev->data->mac_addrs[0]);
...
{
	struct stmmac_private *private = dev->data->dev_private;
	struct stmmac_counters *mmc = &private->mmc;
	struct rte_eth_stats *eth_stats = &private->stats;

	stmmac_mmc_read(private, private->mmcaddr, mmc);
	if (mmc->mmc_rx_framecount_gb >= eth_stats->ipackets)
		eth_stats->imissed = mmc->mmc_rx_framecount_gb - eth_stats->ipackets;
...
static int
stmmac_stats_reset(struct rte_eth_dev *dev)
{
	struct stmmac_private *private = dev->data->dev_private;
	struct rte_eth_stats *eth_stats = &private->stats;
...
stmmac_init_rx_buffers(struct stmmac_private *priv, struct dma_desc *p,
		       struct rte_mbuf *buf)
{
	stmmac_set_desc_addr(priv, p, rte_cpu_to_le_64(rte_pktmbuf_iova(buf)));
...
static int
stmmac_mtu_set(struct rte_eth_dev *dev, uint16_t mtu)
{
	RTE_SET_USED(dev);
	RTE_SET_USED(mtu);
...
stmmac_hw_config_init(struct stmmac_private *priv)
{
	...
	priv->plat->dma_cfg.mixed_burst = 0;
	priv->plat->dma_cfg.eame = 1;

	/* axi */
	priv->plat->axi.axi_wr_osr_lmt = 15;
	priv->plat->axi.axi_rd_osr_lmt = 15;
	priv->plat->axi.axi_blen[4] = 16;
	priv->plat->axi.axi_blen[5] = 0;
	priv->plat->axi.axi_blen[6] = 0;
}
...
pmd_stmmac_probe(struct rte_vdev_device *vdev)
{
	...
	struct rte_ether_addr macaddr = {
		.addr_bytes = { 0xd2, 0x3a, 0xa0, 0x27, 0x2d, 0x3c }
	};
	...
	if (rte_eal_process_type() == RTE_PROC_SECONDARY) {
		...
		if (!dev) {
			STMMAC_PMD_ERR("Failed to attach secondary process for %s\n", name);
			return -1;
		}
	 ...
	 */
	stmmac_store_mac_address(dev, &macaddr);
	rte_eth_dev_probing_finish(dev);
