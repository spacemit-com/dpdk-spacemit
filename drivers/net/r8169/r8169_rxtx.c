...
rtl_recv_pkts(void *rx_queue, struct rte_mbuf **rx_pkts, uint16_t nb_pkts)
{
	...
	while (nb_rx < nb_pkts) {
...
#if defined(RTE_SOC_SPACEMIT_K1) || defined(RTE_SOC_SPACEMIT_K3)
		{
			...
			new_data = rte_pktmbuf_mtod(new_mb, uint8_t *);
			flush_len = new_mb->buf_len - rte_pktmbuf_headroom(new_mb);
			for (i = 0; i < flush_len; i += 64) {
				cbo_flush(new_data + i);
			}
		}
#endif
	     ...
	     struct rte_mbuf *tx_pkt)
{
	...
	for (m_seg = tx_pkt; m_seg; m_seg = m_seg->next) {
...
#if defined(RTE_SOC_SPACEMIT_K1) || defined(RTE_SOC_SPACEMIT_K3)
		{
			...
			offset = (uintptr_t)data & 63ULL;
			for (i = 0; i < (len + offset); i += 64) {
				cbo_flush((uint8_t *)data - offset + i);
			}
			rte_wmb();
		}
#endif
