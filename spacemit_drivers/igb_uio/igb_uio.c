...
/**
 * @brief Allocate non-coherent DMA memory for TX/RX descriptor rings via UIO maps.
 * 
 * Since the Spacemit K1/K3 platform lacks hardware-enforced I/O cache coherency,
 * this function uses dma_alloc_coherent() to allocate hardware-safe, un-cached,
 * physically contiguous memory for descriptors. The physical addresses (IOVA) 
 * are then exposed to DPDK user-space via UIO memory mappings (sysfs uio resource n).
 *
 * @param dev      Pointer to the PCI device.
 * @param info     Pointer to the UIO information structure.
 * @param n        The index of the UIO memory map/resource slot.
 * @param name     Identifier string for the ring (e.g., "TX", "RX").
 * @return int     0 on success, negative error code on failure.
 */
static int
igbuio_alloc_coherent_desc_ring(struct pci_dev *dev, struct uio_info *info,
		       int n, const char *name)
{
	...
	pr_info("%s %d name: %s, addr: 0x%lx, len: 0x%lx [%d]\n", __func__, __LINE__, name, (unsigned long)PhyAddr, (unsigned long)info->mem[n].size, n);
		       ...
		       int n, int pci_bar, const char *name)
{
	...
	info->mem[n].size = len;
	info->mem[n].memtype = UIO_MEM_PHYS;
	pr_err("%s %d name: %s, addr: %lx, len: 0x%lx\n", __func__, __LINE__, name, addr, len);
...
igbuio_setup_bars(struct pci_dev *dev, struct uio_info *info)
{
	...
	iom++;
	ret = igbuio_alloc_coherent_desc_ring(dev, info, iom, "DESCSD");
	iom++;
	ret = igbuio_alloc_coherent_desc_ring(dev, info, iom, "DESCPD");
	*/
